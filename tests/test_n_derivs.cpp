...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_e.deta - diff) / mp::abs(dn_e.deta);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.deta - diff) / mp::abs(dn_pos.deta);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_e.dbeta - diff) / mp::abs(dn_e.dbeta);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.dbeta - diff) / mp::abs(dn_pos.dbeta);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_e.deta2 - diff) / mp::abs(dn_e.deta2);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.deta2 - diff) / mp::abs(dn_pos.deta2);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t err_eta = mp::abs(dn_e.detadbeta - diff_eta) / mp::abs(dn_e.detadbeta);

                // check ∂²n⁻/∂η∂β by differencing ∂²n⁻/∂η∂β

                const real_t _h2 = h * mp::abs(beta);
                ...
                real_t err_beta = mp::abs(dn_e.detadbeta - diff_beta) / mp::abs(dn_e.detadbeta);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                if (mp::abs(dn_pos.detadbeta) < pos_thresh) {
                    continue;
                }
                ...
                real_t err_eta = mp::abs(dn_pos.detadbeta - diff_eta) / mp::abs(dn_pos.detadbeta);
                ...
                real_t err_beta = mp::abs(dn_pos.detadbeta - diff_beta) / mp::abs(dn_pos.detadbeta);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_e.dbeta2 - diff) / mp::abs(dn_e.dbeta2);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.dbeta2 - diff) / mp::abs(dn_pos.dbeta2);
...

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                // check ∂³n⁻/∂η³ by differencing on ∂n⁻/∂η
                auto [diff, err] =
                    fd::adaptive_diff2<real_t>([=] (real_t _eta) -> real_t
                    {
                        const real_t eta_tilde_tmp = -_eta - 2.0_rt / beta;
                ...
                real_t rel_err = mp::abs(dn_e.deta3 - diff) / mp::abs(dn_e.deta3);

                util::threshold_println(rel_err,
                                        "eta = {:9.3f}, beta = {:8.3g},  ∂³n⁻/∂η³ = {:12.5g},  error (D2(∂n⁻/∂η)) = {:12.5g}",
                                        eta, beta, dn_e.deta3, rel_err);
            }
        }
    }

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.deta3 - diff) / mp::abs(dn_pos.deta3);

                util::threshold_println(rel_err,
                                        "eta = {:9.3f}, beta = {:8.3g},  ∂³n⁺/∂η³ = {:12.5g},  error (D2(∂n⁺/∂η)) = {:12.5g}",
                                        eta, beta, dn_pos.deta3, rel_err);
            }
        }
    }

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                // check ∂³n⁻/∂η²∂β by differencing on ∂n⁻/∂β
                auto [diff, err] =
                    fd::adaptive_diff2<real_t>([=] (real_t _eta) -> real_t
                    {
                        const real_t eta_tilde_tmp = -_eta - 2.0_rt / beta;
                ...
                real_t rel_err = mp::abs(dn_e.deta2dbeta - diff) / mp::abs(dn_e.deta2dbeta);

                util::threshold_println(rel_err,
                                        "eta = {:9.3f}, beta = {:8.3g},  ∂³n⁻/∂η²∂β = {:12.5g},  error (D2(∂n⁻/∂β)) = {:12.5g}",
                                        eta, beta, dn_e.deta2dbeta, rel_err);
            }
        }
    }

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.deta2dbeta - diff) / mp::abs(dn_pos.deta2dbeta);

                util::threshold_println(rel_err,
                                        "eta = {:9.3f}, beta = {:8.3g},  ∂³n⁺/∂η²∂β = {:12.5g},  error (D2(∂n⁺/∂β)) = {:12.5g}",
                                        eta, beta, dn_pos.deta2dbeta, rel_err);
            }
        }
    }

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_e.detadbeta2 - diff) / mp::abs(dn_e.detadbeta2);

                util::threshold_println(rel_err,
                                        "eta = {:9.3f}, beta = {:8.3g},  ∂³n⁻/∂η∂β² = {:12.5g},  error (D(∂²n⁻/∂β²)) = {:12.5g}",
                                        eta, beta, dn_e.detadbeta2, rel_err);
            }
        }
    }

    {
        ...
        for (auto eta : {-10.0_rt, 0.0_rt, 50.0_rt, 500.0_rt, 10000.0_rt}) {
            for (auto beta : {1.e-6_rt, 1.e-3_rt, 30.0_rt, 100.0_rt}) {
                ...
                real_t rel_err = mp::abs(dn_pos.detadbeta2 - diff) / mp::abs(dn_pos.detadbeta2);

                util::threshold_println(rel_err,
                                        "eta = {:9.3f}, beta = {:8.3g},  ∂³n⁺/∂η∂β² = {:12.5g},  error (D(∂²n⁺/∂β²)) = {:12.5g}",
                                        eta, beta, dn_pos.detadbeta2, rel_err);
            }
        }
    }
